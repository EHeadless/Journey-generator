...
When writing user stories for Journey Generator, follow these rules:
- User types are: "strategist", "CRM strategist", "product strategist", "AI strategist" — not "user"
- Features must trace to the hierarchy: journey phase → demand space → circumstance → activation
- Reference the experience type (product, marketing, service) when relevant
- Demand space stories must include the "remove the product" test in acceptance criteria
- Activation output stories must specify which artifact they produce (feature backlog, CRM journey, or agent spec)
- Reference .claude/skills/ folder for prompt-related stories
